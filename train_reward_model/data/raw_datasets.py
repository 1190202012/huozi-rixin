...
class PromptRawDataset(object):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        if dataset_name not in ['local/jsonfile', 'en_reward_model_dataset', 'zh_reward_model_dataset']:
...
# en_reward_model_dataset
class EnRewardModelDataset(PromptRawDataset):
    
    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "en_reward_model_dataset"
        self.dataset_name_clean = "en_reward_model_dataset"
        
        raw = load_dataset("openai/webgpt_comparisons", split="train").filter(lambda i: i["question"]["dataset"] == "eli5" and i["answer_0"].strip() != "" and i["answer_1"].strip() != "" and i["score_0"] != i["score_1"])
        self.raw_datasets = raw
        
        query_index_dict = {}

        for i, sample in enumerate(raw):
            question = sample["question"]["full_text"] 
            if question not in query_index_dict.keys():
                query_index_dict[question] = []
            query_index_dict[question].append(i)

        train_dataset = []
        eval_dataset = []
        eval_query = []

        for query in query_index_dict.keys():
            if len(eval_dataset) / len(raw) < 0.1:
                for i in query_index_dict[query]:
                    eval_dataset.append(raw[i])
                    eval_query.append({"query":truncate_en_doc(query, 300, 0).strip()})
            else:
                for i in query_index_dict[query]:
                    train_dataset.append(raw[i])
        
        with open("eval_query.jsonl", "w", encoding = "UTF-8") as f:
            for query in eval_query:
    ...
    def get_prompt(self, sample):
        assert False, "This method is not found usage"

    def get_equal_flag(self, sample):
    ...
    def get_chosen(self, sample):
        if sample["score_0"] > sample["score_1"]:
            answer = sample["answer_0"]
        elif sample["score_0"] < sample["score_1"]:
            answer = sample["answer_1"]
        else:
            # assert False, "Scores of chosen and reject should not be equal"
            answer = sample["answer_0"]
        
        # Until now we are not prepare to cite reference, so remove cite like "[1]". 
    ...
    def get_prompt_and_chosen(self, sample):
        ...
        query = truncate_en_doc(sample["question"]["full_text"], 300, 0).strip()
...
class ZhRewardModelDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank,dataset_name)
        self.dataset_name = "zh_reward_model_dataset"
        self.dataset_name_clean = "zh_reward_model_dataset"

        self.train_dataset = load_dataset("beyond/rlhf-reward-single-round-trans_chinese", split="train")
        self.eval_dataset = load_dataset("beyond/rlhf-reward-single-round-trans_chinese", split="test")

        self.sys = "请对助手回复的有用性、事实性进行打分。\n\n"
    ...
    def get_prompt(self, sample):
        return self.sys + "人类：" + sample['prompt'] + "\n\n"

    def get_chosen(self, sample):
        return "助手：" + sample['chosen']

    def get_rejected(self, sample):
        return "助手 " + sample['rejected']
