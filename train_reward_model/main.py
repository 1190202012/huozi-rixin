...
from model.model_utils import create_critic_model
from data.data_utils import create_prompt_dataset, DataCollatorReward
...
def main():
    ...
    def evaluation_reward(model, eval_dataloader, fast=True):
        model.eval()
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            if fast and step == 99:  # For faster evaluation and debugging
                break
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            print_rank_0(f"epoch: {epoch}, step: {step}, loss: {loss.item()}",args.global_rank)
            ...
            if step % 200 == 0 and step != 0:
                ...
                print_rank_0(
                    f"chosen_last_scores (higher is better) : {reward_score}, acc (higher is better) : {acc}",
                    args.global_rank)
                if args.output_dir is not None and acc > best_batch_acc:
                    print_rank_0('saving model ...', args.global_rank)
                    rm_model = convert_lora_to_linear_layer(rm_model)

                    if args.global_rank == 0:
                        save_hf_format(rm_model, tokenizer, args, "batch_selection")
        ...
        #     args.global_rank)
        # reward_score, acc = evaluation_reward(rm_model, eval_dataloader, fast=False)
        # print_rank_0(
        #     f"chosen_last_scores (higher is better) : {reward_score}, acc (higher is better) : {acc}",
        #     args.global_rank)
        # rm_model.tput_timer.update_epoch_count()

        # if args.output_dir is not None and acc > best_acc:
        #     print_rank_0('saving model ...', args.global_rank)
        #     rm_model = convert_lora_to_linear_layer(rm_model)
